...
typedef struct ProbabilityLex {
    // поле, содержащую информацию о текущей лексеме
    // (тип, символ, строка и другие)
    Lextok* lex;

    // флаг, обозначающий является ли
    // текущая лексема (которая представляет собой ветку оператора if)
    // "вероятностной", то есть, задана ли для нее вероятность.
	int isProb;

	// значение вероятности
	int probVal;

	// флаг, описывающий задана ли вероятность через переменную,
	int isExpr;
} ProbabilityLex;
